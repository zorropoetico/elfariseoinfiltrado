...
import { 
  ...
  Sparkles, Trophy, AlertTriangle, Flame, PlusCircle, Trash2, Save, Upload, RefreshCw, LogOut, Loader2, CheckSquare, Square, PenLine, ClipboardCopy, CheckCircle, XCircle, Copy, Download, Instagram
} from 'lucide-react';
...
// Estructura: createWord(termino, referencia, texto_contenido, activo)
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans selection:bg-indigo-500 selection:text-white pb-10">
      ...
      <main className="max-w-md mx-auto p-4 relative z-0">
        ...
        {/* Backup Modal */}
        <Modal isOpen={showBackupModal} onClose={() => setShowBackupModal(false)} title="Guardar" triggerSound={triggerSound} zIndex="z-[100]">
            <div className="space-y-4">
               <div className="flex items-center gap-3 bg-indigo-50 p-3 rounded-lg border border-indigo-200">
                    ...
                    <p className="text-sm text-slate-700">El código de guardado ha sido generado. Cópialo y guárdalo en tus notas para no perder tus datos.</p>
                </div>
                <textarea 
        ...
        <Modal isOpen={showInfo} onClose={() => setShowInfo(false)} title="Información" triggerSound={triggerSound}>
           <div className="flex space-x-2 border-b border-slate-200 mb-4 pb-1 overflow-x-auto">
               ...
               <button onClick={() => setInfoTab('backup')} className={`px-3 py-2 font-bold text-sm ${infoTab === 'backup' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-400'}`}>Cargar</button>
           ...
           {infoTab === 'credits' ? (
               ...
               <div className="space-y-4">
                   <p className="text-xs text-slate-500">Pega aquí tu código de guardado para cargar el progreso.</p>
                   <textarea 
                        className="w-full h-32 bg-slate-100 text-slate-900 text-xs p-2 rounded border border-slate-300 font-mono"
                        value={backupText}
                   ...
                   />
                   <div className="flex gap-2">
                       <button onClick={importData} className="flex-1 bg-green-600 text-white py-2 rounded text-xs font-bold flex items-center justify-center gap-2"><Upload size={14}/> Cargar</button>
                   </div>
               </div>
           ) : (
